...
import (
	...
	"indecentsoftware.com/internal/tint"
)
...
func TestHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{ // https://github.com/lmittmann/tint/issues/8
			F: func(l *slog.Logger) {
				l.Log(t.Context(), slog.LevelInfo+1, "test")
			},
		...
		},
		{
			...
			F: func(l *slog.Logger) {
				l.Log(t.Context(), slog.LevelDebug-1, "test")
			},
...
func TestReplaceAttr(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			slogLogger.Log(t.Context(), slog.LevelInfo, "", test...)
			...
			tintLogger.Log(t.Context(), slog.LevelInfo, "", test...)
...
func TestErr(t *testing.T) {
	t.Run("text", func(t *testing.T) {
		...
		record := slog.NewRecord(faketime, slog.LevelInfo, "test", 0)
		record.AddAttrs(tint.Err(errTest))
	...
	benchmarks := []struct {
	...
	}{
		{
			...
			func(logger *slog.Logger) {
				logger.LogAttrs(b.Context(), slog.LevelInfo, testMessage,
		...
		},
		{
			"5 args custom level",
			func(logger *slog.Logger) {
				logger.LogAttrs(b.Context(), slog.LevelInfo+1, testMessage,
					slog.String("string", testString),
					slog.Int("status", testInt),
					slog.Duration("duration", testDuration),
					slog.Time("time", testTime),
		...
		},
		{
			"error",
			func(logger *slog.Logger) {
				logger.LogAttrs(b.Context(), slog.LevelError, testMessage,
					tint.Err(errTest),
				)
			},
		},
