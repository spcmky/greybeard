...
pub struct ForgeEvent {
    ...
    pub context: Option<u64>, // GitHub installation id; None for GitLab
}
...
pub trait Forge {
    ...
    /// Create or update the single greybeard comment (marker = state).
    async fn upsert_comment(&self, c: &ChangeRef, body: &str) -> Result<String>;
...
into a **forge-agnostic renderer** (diff/blame/comment structs → the deterministic
pack string — this is most of the file and stays shared) and a **forge-specific
fetcher** (the API calls that fill those structs). `comment.rs`'s marker helpers
are already forge-agnostic; only the two mutations are GitHub-specific.

## GitHub → GitLab API mapping

Recommendation: use **GitLab REST v4** for the GitLab backend (simpler and more
uniform than its GraphQL for MR + notes + blame; mirrors how we already use REST
for GitHub diff/contents). Base URL `https://gitlab.com/api/v4`, or
...
| Changed files + diff | REST `/pulls/:n` (diff media type) | `GET /projects/:id/merge_requests/:iid/diffs` (or `/changes`) |
...
| Permalink to a line | `.../blob/:sha/:path#Lnn` | `.../blob/:sha/:path#Lnn` (same shape) |
...
| Draft | `draft: true` | `object_attributes.work_in_progress: true` / title `Draft:` |
...
1. **Groundwork (done).** Forge-neutral config (`GREYBEARD_FORGE`,
   `GREYBEARD_TOKEN`, `GREYBEARD_FORGE_URL`), single `connect` dispatch in
   `src/forge.rs`, this doc. GitHub unchanged.
...
- **Comment prompt permalinks** in the pack use the forge blob-URL shape —
  confirm GitLab's `#L<n>` anchor and multi-line (`#L10-20`) form.
...
- **Rate limits:** GitLab.com REST limits differ from GitHub; the parallel-fetch
  fan-out may need a small concurrency cap per review.
